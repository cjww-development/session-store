...
class ErrorHandler @Inject()() extends HttpErrorHandler with ApiResponse {
  ...
  override def onClientError(request: RequestHeader, statusCode: Int, message: String): Future[Result] = {
    ...
    val requestId: String = request.headers.get("requestId").getOrElse("-")
    statusCode match {
      case status@NOT_FOUND => withFutureJsonResponseBody(status, notFoundErrorBody, notFoundErrorMessage(requestId)) { json =>
  ...
  override def onServerError(request: RequestHeader, exception: Throwable): Future[Result] = {
    implicit val req = RequestBuilder.buildEmptyRequest(request)
    val requestId: String = request.headers.get("requestId").getOrElse("-")
    withFutureJsonResponseBody(INTERNAL_SERVER_ERROR, exception.getMessage, internalServerErrorMessage(requestId)) { json =>
